...
5. A **Structured 3-Layer Avatar Prompt Contract** passed to Agent 3 (`AvatarWalker`) after the post-quiz likeness collection step.

---

## 2. Synthesis Pipeline & CUJ Flow

```mermaid
graph TD
    User[1. Start Quiz Immediately] --> Raw[2. User Answers: MC Choices + Free-form Text]
    Raw --> Calc[3. Phase 1: Deterministic MBTI Calculator]
    Calc -->|4-Letter MBTI Code & Dichotomy Scores| Synthesizer[4. Phase 2: LLM Generative Synthesis Agent]
    Raw -->|Sanitized Free-form Context| Synthesizer
    
    Synthesizer --> Archetype[5. TraitArchetypeNode Payload]
    
    Likeness[6. Post-Quiz Likeness Collection: Photo or Attributes] --> Handoff[7. Phase 3: 3-Layer Avatar Prompt Contract]
    Archetype --> Handoff
    Handoff --> Agent3[8. Agent 3: AvatarWalker Generator]
```
...
### 3.3 Phase 3: 3-Layer Avatar Prompt Contract Handoff (Post-Quiz Likeness Integration)

After the user completes the quiz and submits their post-quiz likeness payload (photo upload OR manual physical attributes selector), Agent 2's output is combined with the likeness payload into the **3-Layer Avatar Prompt Contract** passed to Agent 3 (`AvatarWalker`):
...
layer_3_likeness = f"User physical features: {skin_tone} skin tone, {hair_style} {hair_color} hair, {gender_expression} expression"
...
### Test Case B: Hybrid MC + Freeform Context
- **Input**: Standard choices + freeform text *"I spend 3 hours picking a Netflix movie then fall asleep"*.
- **Expected Output**: Dynamic title incorporating procrastination theme, with `action` model radar score elevated to $>85$.
